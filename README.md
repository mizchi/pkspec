...
```sh
nix run github:mizchi/pkspec/v0.1.9 -- init --dir pkspec
nix run github:mizchi/pkspec/v0.1.9 -- exec -f path/to/Test.pkl
nix profile install github:mizchi/pkspec/v0.1.9
```
...
```nix
{
  inputs.pkspec.url = "github:mizchi/pkspec/v0.1.9";
...
```sh
go install github.com/mizchi/pkspec/cmd/...@v0.1.9
...
| Input | Default | Notes |
| --- | --- | --- |
| `version` | the action ref, falling back to latest release | Accepts `local`, `v0.1.9`, `0.1.9`, `v0`, or `latest`. |
