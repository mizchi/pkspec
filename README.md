...
```sh
nix run github:mizchi/pkspec/v0.1.12 -- init --dir pkspec
nix run github:mizchi/pkspec/v0.1.12 -- exec -f path/to/Test.pkl
nix profile install github:mizchi/pkspec/v0.1.12
```
...
```nix
{
  inputs.pkspec.url = "github:mizchi/pkspec/v0.1.12";
...
```sh
go install github.com/mizchi/pkspec/cmd/...@v0.1.12
...
| Input | Default | Notes |
| --- | --- | --- |
| `version` | the action ref, falling back to latest release | Accepts `local`, `v0.1.12`, `0.1.12`, `v0`, or `latest`. |
