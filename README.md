...
```sh
nix run github:mizchi/pkspec/v0.2.0 -- init --dir pkspec
nix run github:mizchi/pkspec/v0.2.0 -- exec -f path/to/Test.pkl
nix profile install github:mizchi/pkspec/v0.2.0
```
...
```nix
{
  inputs.pkspec.url = "github:mizchi/pkspec/v0.2.0";
...
```sh
go install github.com/mizchi/pkspec/cmd/...@v0.2.0
...
| Input | Default | Notes |
| --- | --- | --- |
| `version` | the action ref, falling back to latest release | Accepts `local`, `v0.2.0`, `0.2.0`, `v0`, or `latest`. |
