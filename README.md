...
```sh
nix run github:mizchi/pkspec/v0.2.1 -- init --dir pkspec
nix run github:mizchi/pkspec/v0.2.1 -- exec -f path/to/Test.pkl
nix profile install github:mizchi/pkspec/v0.2.1
```
...
```nix
{
  inputs.pkspec.url = "github:mizchi/pkspec/v0.2.1";
...
```sh
go install github.com/mizchi/pkspec/cmd/...@v0.2.1
...
| Input | Default | Notes |
| --- | --- | --- |
| `version` | the action ref, falling back to latest release | Accepts `local`, `v0.2.1`, `0.2.1`, `v0`, or `latest`. |
