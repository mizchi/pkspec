...
```sh
nix run github:mizchi/pkspec/v0.1.8 -- init --dir pkspec
nix run github:mizchi/pkspec/v0.1.8 -- exec -f path/to/Test.pkl
nix profile install github:mizchi/pkspec/v0.1.8
```
...
```nix
{
  inputs.pkspec.url = "github:mizchi/pkspec/v0.1.8";
...
| Input | Default | Notes |
| --- | --- | --- |
| `version` | the action ref, falling back to latest release | Accepts `local`, `v0.1.8`, `0.1.8`, `v0`, or `latest`. |
