	...
	plans := []*config.Plan{
		{
			...
			Scenarios: map[string]*config.Scenario{
				...
				"password policy": {
					...
					ID:            &codeID,
					ReviewStatus:  "approved",
					Implementations: []*config.Implementation{{Kind: "code", At: &codePtr}},
				},
				"audit log": {
					Name:         "audit log",
					ID:           &missingID,
	...
	plans := []*config.Plan{
		{
			...
			Scenarios: map[string]*config.Scenario{
				"uploads valid media": {
					...
					ID:            &specID,
					Description:   &desc,
					AudienceNotes: map[string]string{"pm": pmNotes},
					Tags:          []string{"spec", "audience:pm", "upload"},
					Audience:      []string{"pm"},
	...
	plans := []*config.Plan{
		{
			...
			Scenarios: map[string]*config.Scenario{
				...
				"content type check": {
					...
					ReviewStatus:  "approved",
					Contributes:   []string{goalID},
					Implementations: []*config.Implementation{{Kind: "code", At: &impl}},
				},
				"preview metadata": {
